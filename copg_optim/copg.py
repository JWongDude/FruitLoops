...
class CoPG(object):
    #Inputs: Max_params/min_params come from network.py
    #Other Inputs: learning rate (lr), weight decay to be specified 
    ...
    #Info is collected if collect_info = True following call of step() function.
    def getinfo(self):
    ...
    #Inputs: objective, log-probability of player 1 and 2. (See Bilinear Game for use case)
    def step(self, ob_tot,lp1,lp2):         #Equation can be found on pg3, eq (4)

        #Computes gradient/total gradient:
        ...
        #Calculates Conjugate Gradient for Specified Player x or y
        if self.solve_x:
        ...
        #Apply Weight Decay/Regularization to Parameters
        index = 0
        for p in self.max_params:
            if self.weight_decay != 0:
                p.data.add_(- self.weight_decay * p) #Note subtraction
        ...
        for p in self.min_params:
            if self.weight_decay != 0:
                p.data.add_(- self.weight_decay * p) #Note addition
            p.data.add_(- self.lr * cg_y[index: index + p.numel()].reshape(p.shape))
            index += p.numel()
        if index != cg_y.numel():
            raise ValueError('CG size mismatch')

        #More Algorithm Data Collection
        if self.collect_info:
...
class RCoPG(object):
    ...
    def step(self, ob, ob_tot,lp1,lp2):
        ...
        self.solve_x = False if self.solve_x else Trueimport math

#Appears like first 260 lines are duplicated below with no changes.
import time
